...
exports.getProfesoresDetallado = () => {
	return new Promise((resolve, reject) => {
		Profesor.fetchAll({
			withRelated: ["usuario", "director.institucion", "director.usuario", "curso", "curso.alumnos.usuario"]
		})
			.then(admins => {
				resolve(admins.toJSON());
			})
		...
		new Usuario()
			...
			.then(result => {
				...
				if (usuario.rol.id === 3) {
					...
					reject(profesorConstants(email, usuario.rol.descripcion).notSupportedRol);
				}
				visitanteServices
